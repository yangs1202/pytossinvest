...
CLI 도구로 쓰려면 [uv](https://docs.astral.sh/uv/) 로 격리 설치하는 것을 권장합니다:

```bash
uv tool install git+https://github.com/yangs1202/pytossinvest.git
```

`tossinvest` 가 PATH 에 안 잡히면 `uv tool update-shell` 후 셸을 다시 엽니다.

라이브러리로 쓰거나 uv 가 없으면 pip 로도 설치할 수 있습니다:
...
[Claude Code](https://claude.com/claude-code) 와 두 가지 방식으로 연동됩니다.

### 1) Plugin (권장)

이 저장소는 plugin marketplace 를 포함합니다. Claude Code 에서:

```
/plugin marketplace add yangs1202/pytossinvest
/plugin install tossinvest@pytossinvest
```

설치하면 `tossinvest` skill 과 slash command(`/tossinvest:price`, `/tossinvest:holdings`,
`/tossinvest:account`)가 추가됩니다. plugin 은 CLI 를 자동 설치하지 않으므로 위
[설치](#설치) 의 `uv tool install` 을 한 번 실행해 두세요.

plugin 구성: `plugins/tossinvest/` (manifest·skill·commands), 카탈로그: `.claude-plugin/marketplace.json`.

### 2) 단일 Skill (수동)

plugin 없이 skill 만 쓰려면 `skills/tossinvest/SKILL.md` 를 전역 스킬로 링크합니다:

```bash
ln -s "$(pwd)/skills/tossinvest" ~/.claude/skills/tossinvest
```

이후 "삼성전자 현재가 알려줘", "내 토스 주식 잔고 보여줘" 같은 요청에 skill 이 활성화됩니다.
