...
이 플러그인은 `/tossinvest:price`, `/tossinvest:holdings`, `/tossinvest:account` slash command 도 제공한다.

## 사전 준비

1. CLI 설치 (한 번만) — [uv](https://docs.astral.sh/uv/) 로 격리 설치한다:
   ```bash
   uv tool install git+https://github.com/yangs1202/pytossinvest.git
   ```
   uv 가 없으면 먼저 설치한다(`brew install uv` 또는 `curl -LsSf https://astral.sh/uv/install.sh | sh`).
   `tossinvest` 명령이 잡히지 않으면 `uv tool update-shell` 후 셸을 다시 연다.
   (uv 가 정말 불가하면 `pip install git+https://github.com/yangs1202/pytossinvest.git` 로 대체 가능.)
